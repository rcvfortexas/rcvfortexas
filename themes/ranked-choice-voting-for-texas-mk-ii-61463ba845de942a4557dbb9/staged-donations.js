...
$(document).ready(function(){
  ...
  (function(){
    ...
    this.scrollContainerTop = function() {
      ...
      if ($(window).width() > 768) {
        $('html, body').animate({
          scrollTop: progressStages.offset().top - navContainer.height() - 120
        }, 500);
      } else {
        $('html, body').animate({
          scrollTop: progressStages.offset().top - 120
        }, 500);
      }
    }
    ...
    this.updateProgressIndicator = function(indicator, direction) {
      ...
      if (direction && current.next().length) { // direction = 0 if going backwards, 1 if going forwards
        current.addClass('completed').find('.stage-count-inner');
    ...
    this.updateProgressStages = function(stages, direction) {
      ...
      if (direction && current.next().length) { // direction = 0 if going backwards, 1 if going forwards
        current.hide('slide',{direction:'left',easing:'easeInBack'},300,function(){
          ...
          current.next().show('slide',{direction:'left'},300,function(){$(this).addClass('active');StagedDonations.scrollContainerTop()});
        });
      } else if (!direction && current.prev().length) {
        current.hide('slide',{direction:'left',easing:'easeInBack'},function(){$(this).removeClass('active');
          current.prev().show('slide',{direction:'left'},300,function(){$(this).addClass('active');StagedDonations.scrollContainerTop()});
        });
      } else {
        // do nothing
      }
  ...
  if ($('.progress-indicator-stages').length) {
    ...
    $('.progress-stage-button-next').each(function(){$(this).click(function(event){
      ...
      if (isValid) {
        ...
        progressIndicator.find('.active').removeClass('completed').find('.stage-count-inner');
    ...
    progressStages.find('input[type="submit"]').click(function(event){
      progressIndicator.find('.active').addClass('completed').find('.stage-count-inner');
    });
